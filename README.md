...
- 而服务员需要干的事，就是上一阶段的任务。
- 基于上面的猜测，我们需要对代码进行修改：
   - 创建一个`ClientHandler`类，这个类实现了Runnable接口，作用是”服务员“。
   - 修改main方法，让他当好接待员。
     代码：
- ClientHandler.java
``` java
...
1. 为什么要有 Redis 协议解析器？(The "Why")
   ...
   - 而当客户端发送一个命令时，它不会直接以这个字符串的形式发送。它会遵循一种叫做 **RESP (REdis Serialization Protocol)** 的格式，类似：`*4\r\n$3\r\nSET\r\n$7\r\nuser:1\r\n$4\r\nname\r\n$5\r\nAlice\r\n`。
...
- ClientHandler.java：处理事务，需要修改，使用Protocol来处理命令。
...
class DataStore {
    // 将 Map 的值类型从 byte[] 改为 ValueEntry
